   ...
   <!-- <script src="<?php echo base_url(); ?>external/js/function.js"></script> -->
	...
	$(document).on('click','#btn_std_test_result_show',function()
	{
		...
		({
			...
			success : function(responses){
				$('#admin_menu_container').html('<div id="admin_new_students_title"><center style="border-radius:180px;background-color:red;">All Test Results Analysis</center></div>'+responses);
			}
		});
	});
	...
	$(document).on('click','.result_check',function()
	{
		...
		var std_name_by_admin = $(this).attr('std_name');
		popup();
		$.ajax
		({
			type: "POST",
			...
			beforeSend: function()
					{
						$('.modal-header').html('<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button><h4 class="modal-title" style="color:Blue;font-weight:900;">All Test Results of Student Name '+std_name_by_admin+'</h4>');
						$('.modal-dialog').removeClass("modal-sm");
						$('#admin_new_students_title').hide();
					},
			success : function(response){
				...
				$(".each_q_box").each(function(){
					...
					{	
						...
						$('.'+qus_types).each(function(){
							...
							{
								var radio_answer =$(this).find(':checked').val();
								//alert(radio_answer);
